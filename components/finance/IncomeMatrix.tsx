...
import { formatCLP, formatForInput, parseCLP, parseExcelValues } from '@/utils/currency';
import { MONTHS, Month } from '@/lib/finance/types';
...
export default function IncomeMatrix({ year, showCLPFormat = true }: IncomeMatrixProps) {
  ...
  const handleCellKeyDown = useCallback((e: KeyboardEvent<HTMLInputElement>) => {
    switch (e.key) {
      ...
      case 'Tab':
        e.preventDefault();
        // TODO: Implementar navegación con Tab
        saveCellEdit();
        break;
    }
  ...
  const handlePaste = useCallback(async (e: React.ClipboardEvent, categoryId: string, month: number) => {
    ...
    try {
      ...
      if (lines.length === 1) {
        ...
        // Múltiples filas - pegar en matriz
        // TODO: Implementar pegado de matriz completa
        ...
        <table className="w-full">
          ...
          <thead className="bg-gray-50 sticky top-0">
            <tr>
              ...
              {(Object.keys(MONTHS) as Array<keyof typeof MONTHS>).map(monthNum => (
                <th
                  key={monthNum}
